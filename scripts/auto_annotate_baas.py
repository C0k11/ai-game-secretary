"""Auto-label trajectory frames using BAAS UI capture templates.
...
Usage:
    py scripts/auto_annotate_baas.py --sample 100       # spot-check 100 frames
    py scripts/auto_annotate_baas.py                     # full run (all trajectory frames)
    py scripts/auto_annotate_baas.py --threshold 0.78    # tune sensitivity
"""
...
def match_one(frame_bgr: np.ndarray, t: dict, max_instances: int) -> list[dict]:
    ...
    if t["mask"] is not None:
        # Some BAAS PNGs have alpha=0 outside the actual icon; ensure mask
